...
def generate_audio_from_summary(summary_text: str, topic_name: str):
    """
    Generates an audio file (MP3) from the given summary text using Google Text-to-Speech (gTTS).

    Args:
        summary_text (str): The text content to convert to speech.
        topic_name (str): The topic name used to generate the output file name.

    Process:
        - Creates an audio file path using the provided topic name.
        - Uses gTTS to convert text to speech.
        - Saves the generated audio file in MP3 format.
        - Logs success or any encountered errors.

    Raises:
        Logs any exceptions that occur during audio generation and saving.
    """
    try:
        audio_output_path = root_path / f"{topic_name}_summary.mp3"
        tts = gTTS(text=summary_text, lang='en')
        tts.save(audio_output_path)
        logger.info(f"Audio summary saved at: {audio_output_path}")
    except Exception as e:
